...
export default class EventArchive extends Component{
	...
	componentDidMount(){
		getAllNotes().then(a=>this.setState({ events: a.val(), loading: false }))
	}

	renderevents(){
		let table_cols = [{property: "heading", title: "Heading"}, {property: "link", title: "Link"},]; 
		let table_data = [];
		this.arrayevents = [];
		if(!this.state.events){return null;}
		Object.keys(this.state.events).forEach((i, j) => {
			table_data[j] = {
				heading: this.state.events[i].name,
				link: <a href={this.state.events[i].link} target="_blank" rel="noopener noreferrer">{this.state.events[i].link}</a>,
			}
		...
		return (
			<div className="wrapper" style={{padding: "30px 0"}}>	
				<Paper zDepth={1} style={{padding: "25px 30px"}}>
					<h1 style={thin}>
						<span style={{marginRight: "20px"}}>Note Archive</span>
					</h1>
					{body}
				</Paper>
				<Dialog
